	...
	while (error==NO_ERROR && (c=getchar()) != '\n'){			//Mientras que no apriete enter
		...
		if (c>='0' && c<='9'){					
			...
			if (num_type==ENTERO){				// paso de modo decimal a entero
				...
				printf ("No puede ingresar una coma dos veces. Vuelva a ingresar toda la operación:\n");
				CLEAN_BUFFER();
			}
		}
		else if (error==NO_ERROR && ((sign==NEG && first_c==NO_FIRST) || sign==POS)  ){
			error=ERROR; 						// si no, es un caracter erroneo
			printf ("El numero ingresado contiene carecteres invlalidos. Vuelva a ingresar toda la operación:\n");		
			CLEAN_BUFFER();		
		}
		first_c=NO_FIRST;
	}
...
static int lect_op (char*str, char*pop){
	...
	if (error==ERROR){
		printf ("El operador ingresado, no es valido. Por favor vuelva a ingresar toda la operación:");
		CLEAN_BUFFER();
